...
struct Pak {
  ...
  Pak(const intrinsic_cast_t intrinsinc_cast_)
      : intrinsic_cast(intrinsinc_cast_) {}
  ...
  KR8MD_INLINE operator const intrinsic_t &() const { return intrinsic; }

  KR8MD_INLINE operator intrinsic_t &() { return intrinsic; }

  KR8MD_INLINE operator const intrinsic_cast_t &() const {
    return intrinsic_cast;
  }

  KR8MD_INLINE operator intrinsic_cast_t &() { return intrinsic_cast; }
...
struct MaskedPak {
  ...
  KR8MD_INLINE void operator=(const TPak &rhs) { pak = select(mask, pak, rhs); }
};

template <typename T, size_t W>
KR8MD_INLINE MaskedPak<T, W> masked(
    Pak<T, W> &pak, const Pak<typename bool_t<T>::type, W> &mask) {
